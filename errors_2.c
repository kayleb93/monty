 ...
 * 			for stacks/queus smaller than two nodes.
...
int short_stack_error(unsigned int line_number, char *op)
{
	fprintf(stderr, "L%u: can't %s, stacks too short\n", line_number, op);
	return (EXIT_FAILURE);
}
 ...
 * @line_number: Line number in Monty bytecodes file where error occured.
 * @messages: The corresponding error message to print.
 *
 * Return: (EXIT_FAILURE) always.
 */
int pchar_error(nsigned int line_number, char *message)
{
